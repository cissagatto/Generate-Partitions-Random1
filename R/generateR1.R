...
generateR1 <- function(ds,
  ...
  rp1 <- foreach(f = 1:number_folds) %dopar%{  
    ...
    while(k<=num.particoes){
      ...
      write.csv(frequencia1, paste("fold-", f,
                                   "-labels-per-group-partition-",
                                   k, ".csv", sep=""), row.names = FALSE)
      ...
      partition = k
      num.groups = k
      resumePartitions = rbind(resumePartitions, 
                               data.frame(fold, partition, num.groups))
  ...
  cat("\n\n#############################################################")
  cat("\n# RANDOM 1: END FUNCTION generated Random Partitions          #")
  cat("\n###############################################################")
  cat("\n\n\n\n")
  gc()
}
